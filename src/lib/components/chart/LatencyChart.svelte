    ...
    <div class="tui-chart-vertical" style="width: 100%; height: 350px;">
      <div class="tui-chart-display">
        {#each chartData as dataPoint, i}
          <div
            class="tui-chart-value {dataPoint.error ? 'red-168' : 'green-168'}"
            style="height: {Math.min(100, (dataPoint.value / maxLatency) * 100).toFixed(
              1
            )}%; {dataPoint.error ? 'background-color: var(--error-color);' : ''}"
          >
