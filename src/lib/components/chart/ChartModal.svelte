...
<div id="chart-modal" class="tui-modal">
    <div class="tui-window modal-centered cyan-168"
        ...
        <fieldset class="tui-fieldset">
            ...
            {#if $rpcStore.historyData.filter(d => !d.error).length > 0}
              {($rpcStore.historyData.filter(d => !d.error).reduce((sum, d) => sum + d.value, 0) / $rpcStore.historyData.filter(d => !d.error).length).toFixed(2)}
                ms
            {:else}
              ?
            {/if}
            ...
            {#if $rpcStore.historyData.length > 0}
              {(100 - ($rpcStore.historyData.filter(d => d.error).length / $rpcStore.historyData.length * 100)).toFixed(2)}
                %
            {:else}
              ?
            {/if}
